...
public class Test4 extends ActionSupport {
	
	
	public String execute(){
		StudentDAO stuDao = new StudentDAO();
		stuDao.getSession().beginTransaction();
		Query query = stuDao.getSession().createQuery("from Student as s where s.stuname=? order by s.id desc");
		query.setString(0, "liuchuang");
